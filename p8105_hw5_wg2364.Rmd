...
simulation_ <- function(mu, sd = 5, sample_size = 30, iteration = 5000){
  ...
  df <- 
    df %>% 
    mutate(ttest = map(.x = sample_data, ~t.test(.x, mu = mu))) %>% 
...
In general, there is no association between the true value of $\mu$ and the power of the test. This is because, according to the CLT, for a fixed sample size (n = 30 in this case), the distribution of the sample mean will follow this normal distribution:

$$\hat\mu \sim N(\mu,\sigma / \sqrt{30})$$
So, under a fixed significance level, proportion of times the null was rejected will equal to the significance level.

Plot B: Make a plot showing the average estimate of $\hat\mu$ on the y axis and the true value of $\mu$ on the x axis. Make a second plot (or overlay on the first) the average estimate of $\hat\mu$ only in samples for which the null was rejected on the y axis and the true value of on the x axis
...
In general, we can see that there are no significant difference of estimation between two groups. Again, it can be explained by the CLT as they all follow an identical distribution. 